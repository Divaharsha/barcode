...
if (isset($_GET['table']) && $_GET['table'] == 'master_weight') {
    $offset = 0;
    $limit = 10;
    $where = '';
    $sort = 'id';
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $search = $db->escapeString($_GET['search']);
        $where .= "WHERE weight like '%" . $search . "%' OR id like '%" . $search . "%'";
    }
    if (isset($_GET['sort'])){
        $sort = $db->escapeString($_GET['sort']);
    }
    ...
    $sql = "SELECT COUNT(`id`) as total FROM `master_weight`" .$where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row)
        $total = $row['total'];
   
    $sql = "SELECT * FROM master_weight " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
     $db->sql($sql);
    $res = $db->getResult();
    ...
    foreach ($res as $row) {
        ...
        $operate = '<a href="delete-master_weight.php?id=' . $row['id'] . '" class="text text-danger"><i class="fa fa-trash"></i>Delete</a>';
        $tempRow['id'] = $row['id'];
        $tempRow['from_weight'] = $row['from_weight'];
        $tempRow['to_weight'] = $row['to_weight'];
        $tempRow['weight'] = $row['weight'] .'gms';
        $tempRow['operate'] = $operate;
        $rows[] = $tempRow;
    }
    $bulkData['rows'] = $rows;
