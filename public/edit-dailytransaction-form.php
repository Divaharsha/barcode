...
if (isset($_POST['btnEdit'])) {
    ...
    if ($permissions['dailytransaction']['update'] == 1) {
        ...
        if ( !empty($name))
        {
            ...
            $sql = "UPDATE `daily_transaction` SET `goldsmith_master_id`='$name',`date` = '$date', `type` = '$type', `subcategory` = '$subcategory_id', `weight` = '$weight', `stone_weight` = '$stone_weight', `wastage` = '$wastage', `touch` = '$touch', `rate` = '$rate', `gst` = '$gst', `amount` = '$amount', `mc` = '$mc', `purity` = '$purity' WHERE `daily_transaction`.`id` = $ID";
            $db->sql($sql);
				...
				<form id="edit_transaction_form" method="post" enctype="multipart/form-data">
					<div class="box-body">
                           ...
                           <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="exampleInputEmail1">Subategory</label> 
                                        <select id='subcategory_id' name="subcategory_id" class='form-control' required>
                                                    ...
                                                    foreach ($result as $value) {
                                                    ?>
                                                        <option value='<?= $value['name'] ?>' <?=$res[0]['subcategory'] == $value['name'] ? ' selected="selected"' : '';?>><?= $value['name'] ?></option>
                                                    <?php } ?>
                                        </select>
                                    </div>
                                </div>
